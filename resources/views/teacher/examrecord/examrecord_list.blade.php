                        ...
                        <table class="table-detail">
                            ...
                            @foreach($list_data as $v)
                                <tr>
                                    ...
                                    <td>[! isset($subject_list[$v['unit']])?$subject_list[$v['unit']]:'' !]</td>
                                    <td>第[! $v['vol'] !]冊第[! $v['unit'] !]單元試卷[! $v['paper_vol'] !]</td>
                                    <td><a class="link-view" href="[! route('ta.examrecord.view',[$v['id'],$uid]) !]">檢視</a></td>
                                </tr>
                            @endforeach
                        </table>
                    @endif
...
<script>
    ...
    function send()
    {
        ...
        dsc = dsc + '&uid=' + $('#uid').val();
        location.href = "[! route('ta.examrecord.list.page') !]"+dsc;
    }
    ...
    function get_student_data()
    {
        ...
        if(city_code > '' && organization_id > '' && grade > '' && class_val > ''){
            $.ajax({
                url: "[! route('ta.examrecord.student') !]",
                type:'POST',
                dataType: "json",
                data: {
                    _token: '[! csrf_token() !]',
